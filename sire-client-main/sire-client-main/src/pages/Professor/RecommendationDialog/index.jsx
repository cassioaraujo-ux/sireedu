...
const SuggestionDialog = ({ open, onClose, classId, isRevisor = false }) => {
    ...
    const [formData, setFormData] = useState({
        ...
        intelligences: [],
        class_id: '',
        class_ids: [] // Array para o Revisor
    });
    ...
    useEffect(() => {
        if (open) {
            setFormData(prev => ({ ...prev, class_id: classId || '', class_ids: [] })); 
            ...
            if (isRevisor) {
                ...
                api.get('/survey/classes').then(res => {
    ...
    }, [open, classId, fetchEducationalTypes, isRevisor]);
    ...
    const handleSubmit = async () => {
        ...
        if (result.success) {
            ...
            setMessage(isRevisor ? 'Produto criado com sucesso no banco de dados!' : 'Sugestão enviada com sucesso! Aguardando aprovação.');
            setOpenSnack(true);
            setTimeout(() => {
                onClose();
                setFormData({
            ...
            setMessageType('error');
            setMessage(result.message || 'Erro ao processar.');
            setOpenSnack(true);
        }
        setLoading(false);
    };
    ...
    return (
        <Dialog open={open} onClose={onClose} maxWidth="md" fullWidth>
            <DialogTitle>{isRevisor ? "Criar Produto Educacional" : "Sugerir Novo Produto Educacional"}</DialogTitle>
            <DialogContent className="suggestion-dialog-content">
                <p className="suggestion-subtitle">
                    {isRevisor 
                        ? "Crie um novo material e o vincule diretamente com as turmas selecionadas." 
                        : "Contribua com a plataforma sugerindo novos materiais. Sua sugestão passará por uma revisão antes de ficar visível."
                    }
                </p>
                ...
                {isRevisor && (
                    <FormControl fullWidth margin="dense">
                        <InputLabel>Vincular com Turmas (Opcional)</InputLabel>
                        <Select
                            multiple
                            name="class_ids"
                            ...
                            renderValue={(selected) => 
                                selected.map(id => {
                                    ...
                                    return c ? `${c.description} (${c.semester}/${c.year})` : id;
                                }).join(', ')
                            }
                        >
                            {allClasses.map((cls) => (
                                <MenuItem key={cls.id} value={cls.id}>
                                    ...
                                    <ListItemText primary={`${cls.description} (${cls.semester}/${cls.year})`} />
                                </MenuItem>
                            ))}
                        </Select>
                    </FormControl>
            ...
            <DialogActions>
                ...
                <Button 
                    ...
                    disabled={loading}
                >
                    {loading ? <CircularProgress size={24} color="inherit" /> : (isRevisor ? "Criar Produto" : "Enviar Sugestão")}
                </Button>
            </DialogActions>
