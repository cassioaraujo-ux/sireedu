...
class EducationalProduct(models.Model):
    ...
    # Only APPROVED items live here. No status field needed for visibility control.
    name = models.CharField(max_length=255)
    ...
    content_source = models.CharField(max_length=255, default='', blank=True)
    activity_type = models.CharField(max_length=255, default='', blank=True)
    media_format = models.CharField(max_length=255, default='', blank=True)
    educational_code = models.CharField(max_length=255, default='', blank=True)
    
    # Optional: Keep track of who suggested it originally, but it doesn't affect visibility
    suggested_by = models.ForeignKey(User, on_delete=models.SET_NULL, null=True, blank=True, related_name='approved_products')
...
class ProductSuggestion(models.Model):
    ...
    # Suggestions sit here until approved.
    STATUS_CHOICES = [
        ('PENDING', 'Pending'),
        ('REJECTED', 'Rejected'),
        ('APPROVED', 'Approved'),
    ...
    info = models.CharField(max_length=2000)
    link = models.CharField(max_length=255)
    type = models.ForeignKey(EducationalType, on_delete=models.PROTECT)
    
    # We store the styles/intelligences here temporarily
    styles = models.ManyToManyField(StudyOption, related_name="suggested_styles")
    intelligences = models.ManyToManyField(StudyOption, related_name="suggested_intelligences")
    
    # Workflow metadata
    status = models.CharField(max_length=20, choices=STATUS_CHOICES, default='PENDING')
    suggested_by = models.ForeignKey(User, on_delete=models.SET_NULL, null=True, blank=True)
    # Important: We need to know if this was suggested for a specific class so we can create the link later
    suggested_for_class = models.ForeignKey(Class, on_delete=models.SET_NULL, null=True, blank=True)
    created_at = models.DateTimeField(auto_now_add=True)
