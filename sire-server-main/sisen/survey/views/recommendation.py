...
def get_educational_types(request):
    """
    Returns a list of all educational product types (e.g. Books, Videos, Apps).
    """
    types = models.EducationalType.objects.all().values('id', 'name')
...
def suggest_product(request):
    """
    Receives a product suggestion from the frontend.
    If it's a Revisor, saves directly as an approved EducationalProduct and links to classes.
    If it's a Professor, saves as a PENDING ProductSuggestion.
    """
    data = request.data
    try:
        # 1. Get the Type
        ...
        if not type_id:
            return Response({'error': 'Tipo de material é obrigatório.'}, status=status.HTTP_400_BAD_REQUEST)
            
        product_type = models.EducationalType.objects.get(pk=type_id)

        # 2. Check if user is Revisor
        is_revisor = request.user.groups.filter(name__in=['Revisor', 'Admin']).exists()

        if is_revisor:
            # DIRECT CREATION LOGIC FOR REVISOR
            product = models.EducationalProduct.objects.create(
                name=data.get('name'),
                link=data.get('link'),
            ...
            if 'styles' in data and data['styles']:
                styles = models.StudyOption.objects.filter(code__in=data['styles'])
                product.styles.set(styles)

            if 'intelligences' in data and data['intelligences']:
                intels = models.StudyOption.objects.filter(code__in=data['intelligences'])
                product.intelligences.set(intels)
                
            product.save()

            # Handle multiple classes for Revisor
            class_ids = data.get('class_ids', [])
            if class_ids:
                classes = models.Class.objects.filter(id__in=class_ids)
                for c in classes:
                    models.ClassProduct.objects.get_or_create(class_id=c, product=product)

            return Response({'message': 'Produto criado e disponibilizado com sucesso!'}, status=status.HTTP_201_CREATED)

        else:
            # PENDING SUGGESTION LOGIC FOR PROFESSOR
            ...
            suggestion = models.ProductSuggestion.objects.create(
                name=data.get('name'),
                link=data.get('link'),
                info=data.get('description', ''),
                type=product_type,
                status='PENDING', 
                suggested_by=request.user,
                suggested_for_class=sclass
            )

            if 'styles' in data and data['styles']:
                styles = models.StudyOption.objects.filter(code__in=data['styles'])
                suggestion.styles.set(styles)

            if 'intelligences' in data and data['intelligences']:
                intels = models.StudyOption.objects.filter(code__in=data['intelligences'])
                suggestion.intelligences.set(intels)
                
            suggestion.save()

            return Response({'message': 'Sugestão enviada para revisão!'}, status=status.HTTP_201_CREATED)

    except models.EducationalType.DoesNotExist:
        return Response({'error': 'Tipo de material inválido.'}, status=status.HTTP_400_BAD_REQUEST)
